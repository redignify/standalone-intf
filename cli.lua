...
function get_ms(var)
    ...
    if input then
        ...
        if( common.us_tonumber( rate ) > 1.1 ) then
            client:append("fast")
        end
        client:append(math.floor(vlc.var.get( input, var ) / 1000) / 1000 )
    else
        client:append("")
    end
    end
...
commands_ordered = {
    ...
    { "get_ms"; { func = get_ms("time"); help = "seconds (with 3 decimals) elapsed since stream's beginning" } };
